...
export default function CookieConsent() {
  ...
  return <>
    {!consent.preferences && <section role="region" aria-label="Cookie consent" className="fixed inset-x-2 bottom-2 z-[100] mx-auto max-w-5xl rounded-2xl border border-stone-200 bg-white p-3 shadow-2xl sm:inset-x-3 sm:bottom-3 sm:p-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="min-w-0 flex-1"><h2 className="text-sm font-semibold text-stone-900 sm:text-base">{consent.config.banner_title}</h2><p className="mt-1 text-xs leading-5 text-stone-600 sm:text-sm sm:leading-6">{consent.config.banner_description} <Link className="underline" to="/cookie-policy">Cookie Policy</Link> · <Link className="underline" to="/privacy-policy">Privacy Policy</Link></p>{navigator.globalPrivacyControl === true && <p className="mt-1 text-xs font-medium text-emerald-700">Global Privacy Control detected. Tracking choices remain off.</p>}</div><div className="grid grid-cols-3 gap-1.5 sm:gap-2"><Button size="sm" className="px-1 text-[11px] sm:px-3 sm:text-sm" variant="outline" onClick={consent.rejectAll}>Reject optional</Button><Button size="sm" className="px-1 text-[11px] sm:px-3 sm:text-sm" variant="outline" onClick={consent.openPreferences}>Manage choices</Button><Button size="sm" className="bg-[#6f3b49] px-1 text-[11px] hover:bg-[#5d2d3a] sm:px-3 sm:text-sm" onClick={consent.acceptAll}>Accept all</Button></div></div>
    </section>}
